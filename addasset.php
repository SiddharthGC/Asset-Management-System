...
if(!preg_match('/dept/',$_SESSION['Email']))
{
...
<form action="addasset.php" method="post" style="margin-top:-4%;">
...
<p><input name="CostPerUnit" type="text"  placeholder="Cost Per Unit" id ="CostPerUnit" onclick = "EnableAddAsset()"></p>
<p><input name="NumberOfUnits" type="text"  placeholder="Quantity" id ="NumberOfUnits" onclick = "EnableAddAsset()"></p>
<p><input type="submit" name="addasset" class="btn waves-effect waves-light" id = "addasset" value="Add asset" disabled></p>
</fieldset>
</form>
</div>
</body>
...
{
	global $AssetName,$SupplierName,$DepartmentStockRegisterNumber,$Configuration;
	...
	if(!preg_match("/^[0-9]*$/",$DepartmentStockRegisterNumber)|| strlen($DepartmentStockRegisterNumber)>4)
	{
		return "Enter valid DepartmentStockRegisterNumber";
	}
	if(strlen($Configuration)>200)
	{
		return "DepartmentStockRegisterNumber cannot be more than 200 characters";
	}
	return "true";
}
if(isset($_POST['addasset']))
{
   ...
   if($cost >0 && $numberofunits>0)
   {
    ...
    $Email= $_SESSION['Email'];   //"dept_cse@mail.com";
    $Status="Available";
	$flag = false;
