...
$assetssql = "select * from assettype";
$assets = mysqli_query($conn,$assetssql);
$departmentsql = "select * from department";
$departments = mysqli_query($conn,$departmentsql);
?>
<!doctype html>
<html>
<head>
...
$assetget = "select * from assettype";
$assets = mysqli_query($conn,$assetget);
$deptget = "select * from department";
$departments = mysqli_query($conn,$deptget);	
?>
<div style=" margin-left:14%; margin-top:1% ;">
<label style="font-size:23px">Search:</label>
</div>
<div style="position:relative ; margin-top:1% ; margin-left:14% ; margin-right:7.5% ; white-space:nowrap">
  <form id = "movetoassetinfoform" action="searchassets2.php" method="post">	
<select id = "assetname" name="items" style="width:190px ; height:40px">
  <option value="All" style='display:none'>By Items</option>
  <option value="All">All</option>
  <?php foreach($assets as $asset): ?>
  <option value="<?php echo $asset['name'] ?>"><?php echo $asset['name'] ?></option>
  <?php endforeach ?>
</select>
<!--Dropdownlist to choose department name-->
<select id = "departmentname" name="department" style="width:190px ; height:40px ; margin-left:8%">
  <option value="All" style='display:none'>By Department</option>
  <option value="All">All</option>
  <?php foreach($departments as $department): ?>
  <option value="<?php echo $department['Dno'] ?>"><?php echo $department['Dname'] ?></option>
  <?php endforeach ?>
</select>
<input type = "submit" name= "go" id= "go" value ="Search" class="btn waves-effect waves-light" style="margin-left:1%" />
</form>
...
{
 $sql = "SELECT assetID,name,status FROM assets WHERE assetID like 'OB%'"; 
...
<thead>
                <tr> 
                   ...
                   <th style="width: 120px"><label><center>Asset id</center></label></th>
                    ...
                    <th><label><center>Department</center></label></th>
